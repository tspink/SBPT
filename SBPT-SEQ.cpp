...
static void DumpTree(const SequenceNode& node, std::vector<uint64_t>& sequence)
{
	/*for (unsigned int i = 0; i < level; i++) {
		std::cerr << "  ";
	}
	std::cerr << OPCODE_StringShort(node.Opcode) << std::endl;*/
	
	sequence.push_back(node.Opcode);
	
	if (node.Children.size() != 0) {
		for (const auto& child : node.Children) {
			DumpTree(child.second, sequence);
		}
	} else {
		for (const auto& op : sequence) {
			std::cerr << OPCODE_StringShort(op) << "; ";
		}
...
void KernelRoutineExit(KernelDescriptor *descriptor)
{
	...
	if (CurrentFrame->Index >= SKIP_FRAME) {
		...
		DumpTree(CurrentKernel->Root, seq);
...
void InstructionExecuted(VOID *rip, uint64_t opcode, uint64_t opclass)
{
	...
	if (opclass == XED_CATEGORY_UNCOND_BR || opclass == XED_CATEGORY_COND_BR) {
